...
function kahuna_layout_meta_box() {
	global $post;
    	global $kahuna_big;
	$options = $kahuna_big['options'][0];
	$custom = ( get_post_custom( $post->ID ) ? get_post_custom( $post->ID ) : false );
	$layout = ( isset( $custom['_cryout_layout'][0] ) ? $custom['_cryout_layout'][0] : '0' );
	if ( empty($layout) && isset( $custom['_kahuna_layout'][0] ) ) $layout = $custom['_kahuna_layout'][0]; // back compat
    ?>
	<p>
    	...
    	<label id="cryout_layout_default">
            ...
            <span><img src="<?php echo esc_url( get_template_directory_uri() ) ?>/admin/images/0def.png" alt="<?php _e( 'Default Theme Layout', 'kahuna' ); ?>" title="<?php _e( 'Default Theme Layout', 'kahuna' ); ?>" /></span>
        </label>
	</p>
	<?php
} //kahuna_layout_meta_box()
...
function kahuna_save_custom_post_metadata() {
	...
	// Layout - Update
	delete_post_meta( $post->ID, '_kahuna_layout', $layout ); // back compat cleanup
