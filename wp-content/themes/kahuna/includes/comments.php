						...
						<div class="comment-meta">
							...
							<time datetime="<?php comment_time( 'c' );?>" <?php cryout_schema_microdata( 'time' );?>>
								...
								<span class="comment-timediff">
									<?php printf( _x( '%1$s ago', '%s = human-readable time difference', 'kahuna' ), esc_html( human_time_diff( get_comment_time( 'U' ), current_time( 'timestamp' ) ) ) ); ?>
								</span>
...
function kahuna_comments_on() {
    ...
    if ( ( comments_open() || get_comments_number() ) && ! post_password_required() && $kahuna_meta_comment && ! is_single() ) :
			...
			comments_popup_link(
				 ...
				 __( '1 comment', 'kahuna' ),
				sprintf( _n( '%1$s Comment', '%1$s Comments', get_comments_number(), 'kahuna' ), number_format_i18n( get_comments_number() ) ),
				'',
				''
			);
			echo '</span>';
