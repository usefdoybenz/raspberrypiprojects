...
<article id="post-<?php the_ID(); ?>" <?php if ( is_sticky() )  post_class( array('hentry' , 'hentry-featured') ); else post_class( 'hentry' ); cryout_schema_microdata( 'blogpost' ); ?>>
	...
	<div class="article-inner">
		...
		$kahuna_excerpt_mode = 'excerpt'; // default
		if ( $kahunas['kahuna_excerptarchive'] == "full" ) { $kahuna_excerpt_mode = 'content'; }
		if ( is_sticky() && $kahunas['kahuna_excerptsticky'] == "full" ) { $kahuna_excerpt_mode = 'content'; }
		if ( $kahunas['kahuna_excerpthome'] == "full" && ! is_archive() && ! is_search() ) { $kahuna_excerpt_mode = 'content'; }
		if ( false != get_post_format() ) { $kahuna_excerpt_mode = 'content'; }

		switch ( $kahuna_excerpt_mode ) {
			case 'content': ?>
